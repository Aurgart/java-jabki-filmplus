...
public class FilmService {
    ...
    public List<Film> searchFilm(String name, String description, String duration, String localdate, Set<Genre> genres) {
        return films.stream().filter(f -> (!StringUtils.hasText(name) || f.getName().toLowerCase().contains(name.toLowerCase())) && (!StringUtils.hasText(description) || f.getDescription().toLowerCase().contains(description.toLowerCase())) && (!StringUtils.hasText(duration) || Objects.equals(f.getDuration(), Long.parseLong(duration))) && (!StringUtils.hasText(localdate) || Objects.equals(f.getReleaseDate(), LocalDate.parse(localdate))) && (genres == null || f.getGenres().equals(genres))).toList();
    }
}
